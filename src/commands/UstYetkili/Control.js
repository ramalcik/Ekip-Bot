...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ buttons : [ecdagit,tagrol,kayıtsızdagit], embed: ozi})
    ...
    let ozi = message.guild.members.cache.filter(member => !member.roles.cache.has(ayar.etkinlik) || !member.roles.cache.has(ayar.cekilis))
    button.reply.send(`
